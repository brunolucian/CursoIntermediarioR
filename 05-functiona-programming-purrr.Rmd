...
O código acima cria uma função com a finalidade de somar uma unidade em cada elemento de um vetor. A utilização da PF por meio da função `map()` aplica a função desejada em cada elemento do objeto dado, dispensando a necessidade de declararmos um iterador auxiliar (`i`).
...
Também é possível utilizar fórmulas dentro da `map()`, basta colocar um til (`~`) antes da função que será chamada. Feito isso, podemos utilizar o placeholder `.x` para indicar onde deve ser colocado cada elemento de `obj`.
...
Usando fórmulas temos maior flexibilidade, pois podemos declarar, por exemplo, funções anônimas como `~.x+2`), enquanto com as reticências temos maior legibilidade.

## Iterações intermediárias

Em algumas situações é necessário realizar operações com mais de um objeto. Observe o laço a seguir:
...
Como o pacote `purrr` é extremamente consistente, a `map2()` também funciona com reticências, fórmulas e dá acesso ao placeholder `.y` para indicar onde os elementos do segundo vetor devem ir. Poderíamos, por exemplo, transformar `soma_ambos()` em uma função anônima:
...
Essa soma cumulativa é bastante simples, mas também é muito fácil de fazer confusão entre os índices e o *bug* acabar
passando desapercebido. Para evitar esse tipo de situação, podemos utilizar `accumulate()` ou `reduce` (tanto com uma função quanto com uma fórmula). A diferença entre elas é que a primeira mantém os resultados intermediários, enquanto a segunda retorna o resultado final. Primeiramente vamos mostrar a utilização da `accumulate()`:
