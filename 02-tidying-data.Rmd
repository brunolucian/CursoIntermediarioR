...
Ou seja, você não pode desaninhar duas colunas que contêm um número diferente de elementos ao mesmo tempo
...
# Não funciona pq y e z não tem mesmo tamanho
df2 <- tribble(
  ~x, ~y,           ~z,
   1, "a",         3:4,  
   2, LETTERS[12:13],   3
...
```
Isso se aplica ao desaninhar colunas de listas de *dataframes*. Você pode desaninhar várias list-cols, desde que todos os *dataframe* em cada linha tenham o mesmo número de linhas.
